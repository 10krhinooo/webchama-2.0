<#--
  ...
  (a right-aligned "* Required fields" legend in a Bootstrap grid) and renders password/password-confirm
  with a Font Awesome eye icon that this theme never loads (login.css only imports PatternFly), so the
  toggle button rendered empty. This override matches login.ftl instead: a plain header + subtitle, and
  the same hand-built SVG eye-icon toggle for both password fields. Username/email/first/last name still
  go through userProfileCommons so field visibility/validation keeps following the realm's user profile
  config rather than being hand-duplicated here.
-->
